...
def get_pids(trace):
	...
	for pid in sorted(freqs, key=freqs.get, reverse=True):
		print('   %s = %i' % (pid, freqs[pid]))
	
def get_inodes(trace):
	syscall = Syscall(sys_ids)
	freqs = {}
	...
	for inode in sorted(freqs, key=freqs.get, reverse=True):
		print('   %s = %i' % (inode, freqs[inode]))

def get_syscalls(trace):
	syscall = Syscall(sys_ids)
	freqs = {}
	...
	for sys in sorted(freqs, key=freqs.get, reverse=True):
		print('   %s = %i' % (sys, freqs[sys]))

def main():
	parser = argparse.ArgumentParser(description='Find all PIDs, inode numbers or system calls in a KMaldetect tracefile. You can specify multiple search flags.')
	parser.add_argument('--pids', dest='pids', action='store_true', help='search for PIDs')
	parser.add_argument('--inodes', dest='inodes', action='store_true', help='search for inode numbers')
	parser.add_argument('--syscalls', dest='syscalls', action='store_true', help='search for system calls')
	...
	if not (pids or inodes or syscalls) or file == '':
		parser.print_help()
		return
