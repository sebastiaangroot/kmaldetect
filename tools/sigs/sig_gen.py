...
import subprocess, pipes, getopt, sys, os

def usage():
	print('Usage: %s -o <outfile> <command>' % sys.argv[0])
	print('\t-o <signature>:\tThe location of the output signature file')
	print('\t<command>:\tThe (optionally multipart) command to trace')

def execute_strace(command):
	args = []
	args.append('strace')
	args.append('-f')
	args += command
	...
	f.close()
	output = open('.pipefile').read().split('\n')
	#delete .pipefile file here
	return output
...
try:
	...
	for o, a in opts:
		if o in ('-o'):
			outfile = a
	if outfile == None:
		raise getopt.GetoptError('No outfile supplied')
...
output = execute_strace(args)
signature = ''
for i, line in enumerate(output):
	sys_name = line[:line.find('(')]
	try:
