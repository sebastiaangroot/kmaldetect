...
def get_from_inode(trace, inode):
	syscall = Record(sys_ids)

	for item in trace:
		syscall.unpack(item)
		if syscall.inode == inode:
			print(syscall.sys, end=', ')
	print()

def get_from_both(trace, pid, inode):
	syscall = Record(sys_ids)

	for item in trace:
		syscall.unpack(item)
		if syscall.inode == inode and syscall.pid == pid:
			print(syscall.sys, end=', ')
	print()

def get_inode_stats(trace, inode):
	syscall = Record(sys_ids)
	sys_freqs = {}
	pid_freqs = {}
	for item in trace:
		...
		if syscall.inode == inode:
			...
			if not syscall.sys in sys_freqs.keys():
				sys_freqs[syscall.sys] = 1
			else:
				sys_freqs[syscall.sys] += 1

			if not syscall.pid in pid_freqs.keys():
				pid_freqs[syscall.pid] = 1
			else:
				pid_freqs[syscall.pid] += 1

	if len(sys_freqs.keys()) == 0:
...
def get_pid_stats(trace, pid):
	...
	for item in trace:
		...
		if syscall.pid == pid:
			...
			if not syscall.inode in inode_freqs.keys():
				inode_freqs[syscall.inode] = 1
			else:
				inode_freqs[syscall.inode] += 1

	if len(inode_freqs.keys()) == 0:
		print('No matches found')
		return
...
def get_both_stats(trace, pid, inode):
	...
	for item in trace:
		syscall.unpack(item)
		if syscall.pid == pid and syscall.inode == inode:
			if not syscall.sys in sys_freqs.keys():
				sys_freqs[syscall.sys] = 1
			else:
				sys_freqs[syscall.sys] += 1
...
def main():
	...
	parser.add_argument('--pid', dest='pid', default=-1, help='the PID to search for')
	parser.add_argument('--inode', dest='inode', default=-1, help='the inode to search for')
	...
	if pid < 0:
		if stats:
			get_inode_stats(trace, inode)
		else:
			get_from_inode(trace, inode)
	elif inode < 0:
		if stats:
			get_pid_stats(trace, pid)
		else:
			get_from_pid(trace, pid)
	else:
		if stats:
			get_both_stats(trace, pid, inode)
		else:
			get_from_both(trace, pid, inode)
